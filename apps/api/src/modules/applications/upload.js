import { read, utils } from "xlsx";
import { AppError } from "../../lib/errors.js";
import { readJsonBody, readRequestBody } from "../../lib/http.js";
import {
  assertAcceptedUploadCount,
  assertSafeSpreadsheetUpload
} from "../../lib/uploadSecurity.js";

function getContentType(req) {
  return String(req.headers["content-type"] || "");
}

function extractBoundary(contentType) {
  const match = /boundary=(?:"([^"]+)"|([^;]+))/i.exec(contentType);
  return match?.[1] || match?.[2] || null;
}

function parseMultipartHeaders(headerBlock) {
  const headers = {};
  for (const line of headerBlock.split("\r\n")) {
    const separatorIndex = line.indexOf(":");
    if (separatorIndex === -1) continue;
    const name = line.slice(0, separatorIndex).trim().toLowerCase();
    headers[name] = line.slice(separatorIndex + 1).trim();
  }
  return headers;
}

function parseDisposition(value) {
  const output = {};
  for (const segment of String(value || "").split(";")) {
    const [rawKey, rawValue] = segment.split("=");
    const key = rawKey?.trim().toLowerCase();
    if (!key) continue;
    output[key] = rawValue === undefined ? true : rawValue.trim().replace(/^"|"$/g, "");
  }
  return output;
}

function parseMultipart(buffer, contentType) {
  const boundary = extractBoundary(contentType);
  if (!boundary) {
    throw new AppError(400, "Multipart requests must include a boundary.");
  }

  const boundaryToken = `--${boundary}`;
  const raw = buffer.toString("latin1");
  const segments = raw.split(boundaryToken).slice(1, -1);
  const parts = [];

  for (const segment of segments) {
    const trimmed = segment.replace(/^\r\n/, "").replace(/\r\n$/, "");
    if (!trimmed) continue;

    const separatorIndex = trimmed.indexOf("\r\n\r\n");
    if (separatorIndex === -1) continue;

    const headerBlock = trimmed.slice(0, separatorIndex);
    const bodyBlock = trimmed.slice(separatorIndex + 4);
    const headers = parseMultipartHeaders(headerBlock);
    const disposition = parseDisposition(headers["content-disposition"]);
    const body = Buffer.from(bodyBlock, "latin1");

    parts.push({
      headers,
      disposition,
      name: disposition.name || null,
      filename: disposition.filename || null,
      body,
      value: disposition.filename ? null : body.toString("utf8")
    });
  }

  return parts;
}

function normalizeHeader(value) {
  return String(value || "")
    .trim()
    .toLowerCase()
    .replace(/[_./()-]+/g, " ")
    .replace(/\s+/g, " ");
}

function collapse(value) {
  return String(value ?? "").replace(/\s+/g, " ").trim();
}

function detectHeaderRowIndex(matrix) {
  const keywords = ["reference", "student id", "name", "email", "programme", "score", "amount", "remarks"];
  let bestIndex = -1;
  let bestScore = 0;

  for (let rowIndex = 0; rowIndex < Math.min(matrix.length, 25); rowIndex += 1) {
    const row = matrix[rowIndex] || [];
    let score = 0;

    for (const cell of row) {
      const normalized = normalizeHeader(cell);
      for (const keyword of keywords) {
        if (normalized.includes(keyword)) {
          score += 1;
        }
      }
    }

    if (score > bestScore) {
      bestIndex = rowIndex;
      bestScore = score;
    }
  }

  return bestScore >= 2 ? bestIndex : -1;
}

function getMappedValue(headers, row, aliases) {
  for (const alias of aliases) {
    const matchIndex = headers.findIndex((header) => header === alias || header.includes(alias));
    if (matchIndex !== -1) {
      return collapse(row[matchIndex]);
    }
  }
  return "";
}

function rowsFromSheet(sheet, fileName, sheetName) {
  const matrix = utils.sheet_to_json(sheet, {
    header: 1,
    defval: "",
    blankrows: false,
    raw: false
  });

  if (!matrix.length) return [];

  const headerRowIndex = detectHeaderRowIndex(matrix);
  if (headerRowIndex === -1) return [];

  const normalizedHeaders = (matrix[headerRowIndex] || []).map((value) => normalizeHeader(value));
  const rows = [];

  for (let rowIndex = headerRowIndex + 1; rowIndex < matrix.length; rowIndex += 1) {
    const row = (matrix[rowIndex] || []).map((value) => String(value ?? "").trim());
    if (row.every((value) => !value)) continue;

    const studentReferenceId = getMappedValue(normalizedHeaders, row, [
      "reference number",
      "reference no",
      "ref no",
      "student id"
    ]);
    const fullName = getMappedValue(normalizedHeaders, row, [
      "full name",
      "student name",
      "name"
    ]);

    if (!studentReferenceId && !fullName) {
      continue;
    }

    rows.push({
      "Reference Number": studentReferenceId,
      "Full Name": fullName,
      "Email Address": getMappedValue(normalizedHeaders, row, [
        "email address",
        "email",
        "applicant email",
        "student email"
      ]),
      "Programme of Study": getMappedValue(normalizedHeaders, row, [
        "programme of study",
        "program of study",
        "programme",
        "program"
      ]),
      "Current Year": getMappedValue(normalizedHeaders, row, ["current year", "year", "level"]),
      "Average Score": getMappedValue(normalizedHeaders, row, [
        "average score",
        "final score",
        "score"
      ]),
      Remarks: getMappedValue(normalizedHeaders, row, ["remarks", "comment", "need category"]),
      Notes: `${fileName} / ${sheetName}`
    });
  }

  return rows;
}

function rowsFromWorkbook(buffer, fileName) {
  const workbook = read(buffer, { type: "buffer", raw: false });
  const rows = [];

  for (const sheetName of workbook.SheetNames) {
    rows.push(...rowsFromSheet(workbook.Sheets[sheetName], fileName, sheetName));
  }

  if (!rows.length) {
    throw new AppError(400, `The uploaded workbook ${fileName} does not contain recognizable application rows.`);
  }

  return rows;
}

function rowsFromCsv(file) {
  const workbook = read(file.body, { type: "buffer", raw: false });
  const sheet = workbook.Sheets[workbook.SheetNames[0]];
  return utils.sheet_to_json(sheet, { defval: "", raw: false });
}

function rowsFromUploadedFile(file) {
  const { extension, fileName } = assertSafeSpreadsheetUpload(file, "application imports");

  if (fileName.startsWith("~$")) {
    return null;
  }

  return {
    fileName,
    fileType: extension === ".csv" ? "csv" : "xlsx",
    rows: extension === ".csv" ? rowsFromCsv(file) : rowsFromWorkbook(file.body, fileName)
  };
}

function mergeResults(results) {
  const validResults = results.filter(Boolean);
  const rows = validResults.flatMap((result) => result.rows);

  if (!rows.length) {
    throw new AppError(400, "No importable application rows were found in the uploaded files.");
  }

  return {
    fileName: validResults.map((result) => result.fileName).join(", "),
    fileType: validResults.length > 1 ? "multi-file" : validResults[0].fileType,
    rows
  };
}

export async function resolveApplicationImportPayload(req, maxBytes) {
  const contentType = getContentType(req);
  const normalizedContentType = contentType.toLowerCase();

  if (normalizedContentType.includes("application/json")) {
    const payload = await readJsonBody(req, maxBytes);
    return {
      source: "json",
      fileName: payload.fileName || null,
      fileType: "json",
      rows: Array.isArray(payload.rows) ? payload.rows : [],
      schemeId: payload.schemeId || "",
      cycleId: payload.cycleId || "",
      importMode: payload.importMode || "applications"
    };
  }

  if (normalizedContentType.includes("multipart/form-data")) {
    const body = await readRequestBody(req, maxBytes);
    const parts = parseMultipart(body, contentType);
    const files = parts.filter((part) => part.filename && (part.name === "file" || part.name === "files"));

    if (!files.length) {
      throw new AppError(400, "Multipart application imports must include one or more file fields.");
    }
    assertAcceptedUploadCount(files, "Application import");

    const getTextPart = (name) => parts.find((part) => !part.filename && part.name === name)?.value?.trim() || "";

    return {
      source: "upload",
      schemeId: getTextPart("schemeId"),
      cycleId: getTextPart("cycleId"),
      importMode: getTextPart("importMode") || "applications",
      ...mergeResults(files.map(rowsFromUploadedFile))
    };
  }

  throw new AppError(415, "Application import requests must use application/json or multipart/form-data.");
}
